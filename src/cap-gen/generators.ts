import * as s from "../cap/std/schema.capnp.js";
import { format } from "../cap/util";
...
export function generateNestedImports(ctx: CodeGeneratorFileContext): void {
	ctx.imports.forEach((i) => {
		...
		if (name.substr(0, 7) === "/capnp/") {
			importPath = `../cap/std/${name.substr(7)}.js`;
		} else {
...
export function generateFileId(ctx: CodeGeneratorFileContext): void {
	...
	const fileId = f.createCallExpression(BIGINT, __, [
...
export function generateStructFieldMethods(
	...
	if (adopt) {
		const parameters = [
			f.createParameterDeclaration(__, __, __, VALUE, __, orphanType, __),
		];
		const expressions = [
	...
	if (hasUnnamedUnion) {
		...
		members.push(
			createMethod(
				"which",
				[],
				f.createTypeReferenceNode(`${fullClassName}_Which`, __),
				[whichExpression],
				true,
			),
		);
